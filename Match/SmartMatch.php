...
class SmartMatch implements Matcher
{
        ...
        foreach ($this->router->getConfig()['viewExtension'] as $extension) {
            if (is_file(rtrim($this->router->getConfig()['viewPath'] . $this->router->route->getUrl() . $extension, '/'))) {
                ...
                for ($i = 0; $i < $this->router->collection->countRoutes; ++$i) {
                    if (is_file($this->router->collection->routes['path_' . $i] . $this->router->getConfig()['viewPath'] . $this->router->route->getUrl() . $extension)) {
                        $this->router->route->setTarget(['dispatcher' => 'JetFire\Routing\Dispatcher\TemplateDispatcher', 'template' => $this->router->collection->routes['path_' . $i] . $this->router->getConfig()['viewPath'] . $this->router->route->getUrl() . $extension, 'extension' => str_replace('.', '', $extension)]);
                        $this->router->route->addDetail('block', $this->router->collection->routes['path_' . $i]);
                        return true;
                    }
                }
            }
    ...
    private function matchMvc()
    {
        ...
        for ($i = 0; $i < $this->router->collection->countRoutes; ++$i)
            if ('/' . $routes[0] == $this->router->collection->routes['prefix_' . $i])
                $route = array_slice($routes, 1);
        if (isset($route[0])) {
            $block = isset($this->router->collection->routes['path_' . $i]) ? $this->router->collection->routes['path_' . $i] : '';
