...
class RoutesMatch implements Matcher
{
    ...
    public function match()
    {
        ...
        for ($i = 0; $i < $this->router->collection->countRoutes; ++$i) {
            $this->request['block'] = $this->router->collection->routes['path_' . $i];
            $this->request['prefix'] = (isset($this->router->collection->routes['prefix_' . $i])) ? $this->router->collection->routes['prefix_' . $i] : '';
            foreach ($this->router->collection->routes['routes_' . $i] as $route => $dependencies) {
    ...
    public function template()
    {
        if (!$this->router->route->hasTarget()) {
            ...
            if (in_array('.' . $extension, $this->router->getConfig()['viewExtension'])) {
                ...
                if (!empty($target)) {
                    $this->router->route->setTarget(['dispatcher' => 'JetFire\Routing\Dispatcher\TemplateDispatcher', 'template' => $target, 'extension' => $extension]);
                    return true;
                }
            } else {
                foreach ($this->router->getConfig()['viewExtension'] as $ext) {
                    ...
                    elseif (is_file($path . $ext))
                        $target = $path . $ext;
                    if (!empty($target)) {
                        $this->router->route->setTarget(['dispatcher' => 'JetFire\Routing\Dispatcher\TemplateDispatcher', 'template' => $target, 'extension' => str_replace('.', '', $ext)]);
                        return true;
                    }
                }
                throw new \Exception('Template file "' . $path . '" is not found in "' . $this->router->getConfig()['viewPath'] . '" or "' . $this->router->route->getBlock() . $this->router->getConfig()['viewPath'] . '"');
