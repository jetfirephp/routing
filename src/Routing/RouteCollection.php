...
class RouteCollection
{
    ...
    public function addRoutes($routes = null, $options = [])
    {
        if ($routes !== null && !is_array($routes)) {
            ...
            if (is_file($routes . '/routes.php') && is_array($routesFile = include $routes . '/routes.php')) {
                ...
                throw new \InvalidArgumentException('Argument for "' . static::class . '" constructor is not recognized. Expected argument array or file containing array but "' . $routes . '" given');
            }
        }
    ...
    public function setPrefix($args)
    {
        if (is_array($args)) {
            foreach ($args as $i => $iValue) {
                $this->routes['prefix_' . $i] = '/' . trim($iValue, '/');
            }
    ...
    public function generateRoutesPath($root = null, $script_file = 'index.php')
    {
        ...
        if ($domain !== null && strpos($domain, $new_domain) !== false) {
    ...
    public function getRoutePath($name, $params = [])
    {
        foreach ($this->routesByName as $key => $route) {
            ...
            if (isset($params['{subdomain}'])) {
                $route = str_replace('*', $params['{subdomain}'], $route);
            }
            if (isset($params['{method}'])) {
                $route = str_replace('*', $params['{method}'], $route);
            }
